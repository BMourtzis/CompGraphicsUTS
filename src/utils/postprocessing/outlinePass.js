import { Vector2, Vector3, Color, MeshBasicMaterial, MeshDepthMaterial, ShaderMaterial, WebGLRenderTarget, UniformsUtils, OrthographicCamera, Scene, PlaneBufferGeometry, Matrix4, Mesh, LinearFilter, RGBAFormat, DoubleSide, RGBADepthPacking, NoBlending, AdditiveBlending } from "three";
  ...
  render(renderer, writeBuffer, readBuffer, delta, maskActive) {
    if (this.selectedObjects.length > 0) {
      ...
      if (this.pulsePeriod > 0) {
        ...
        let scalar = (1 + 0.25) / 2 + Math.cos(performance.now() * 0.01 / this.pulsePeriod) * (1.0 - 0.25) / 2;
